...
+ GPIO 中断采样 RX 信号。如需使用硬件 UART，关闭该选项即可。

> ⚠️ 本分支（传感器整合修改）的 `sw_uart.c` 采样时序每位只前进半个位周期，可能导致字节错乱；该问题在 `传感器修改2` 修复。另外 `sw_uart.c` 缺 `#include <string.h>`、组件 `REQUIRES` 缺 `esp_driver_gpio`，在 `传感器修改3` 才补齐。实际使用建议用 `传感器修改3/4` 分支。
