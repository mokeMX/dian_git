...
> 🚨 **冲突警告（本分支为原始整合版）**：下面给出的是「每个传感器单独接线」的推荐值。把它们**放在一起会冲突**——A02YYUW(UART1)、BU_UWB(UART1)、RPLIDAR(UART2) 的默认 RX/TX **都是 GPIO36/37**，而 I2C 默认 SCL 也用到 GPIO37。因此**默认配置下不能多个串口传感器同时启用**，I2C 与串口也会争用 GPIO37。
>
> - 单独验证某一个传感器时，本表可用。
> - 要多传感器同时运行，请在 `menuconfig` 把各传感器引脚改到互不冲突，或直接使用已解决冲突的 `传感器修改4` / `算法2` 分支（IO35/36/37 三个真正独立引脚 + 软件 UART 方案）。
