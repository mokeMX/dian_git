...
void app_main(void)
{
    ...
    /* ---- A02YYUW #2 (IO36, SW-UART) ----------------------------*/
    a02yyuw_config_t a02b_cfg = a02yyuw_default_config((uart_port_t)A02_2_UART_PORT, A02_2_RX_GPIO, A02_2_TX_GPIO);
    ...
    if (lidar_ret == ESP_OK) {
        ...
        if (ret_i == ESP_OK) {
            printf("[RPLIDAR][INFO] model=%u.%u firmware=%u.%u hardware=%u sn=%s\n",
                   info.major_model, info.sub_model, info.firmware_major, info.firmware_minor, info.hardware, info.serial_num);
        } else {
    ...
    if (imu_ret == ESP_OK) {
        ...
        if (ret_v == ESP_OK) {
            printf("[IMU] version=%u.%u.%u addr=0x%02X SDA=GPIO%d SCL=GPIO%d\n",
                   version[0], version[1], version[2], imu_cfg.device_address, imu_cfg.sda_gpio, imu_cfg.scl_gpio);
        } else {
...
/* ---- launch all sensor tasks -------------------------------*/
    // 将两个超声波任务固定在 Core 0，并且将优先级提高到 4 (最高)
...
//  * ================================================================ */
// // 【修改】：将 I2C 移出可能发生内部冲突的 38/39，换到普通 IO 15 和 16
// #define HUB_I2C_PORT           0
// #define HUB_I2C_SDA_GPIO      38 
// #define HUB_I2C_SCL_GPIO      39
// #define HUB_I2C_SPEED_HZ      400000
...
// // g_a02_1 和 g_a02_2 的全局句柄不再需要，因为我们在局部任务中动态创建
// static rplidar_c1_t g_lidar;
// static imu_i2c_t g_imu;
// static vl53l1x_tof_t g_tof;
// static volatile bool g_lidar_scan_active;
...
//  * Sensor tasks
//  * ================================================================ */
// /* ---- A02YYUW 共享分时读取任务 (抗干扰版) ----------------------*/
// static void task_a02yyuw_shared(void *pvParameters)
// {
//     (void)pvParameters;
//     a02yyuw_t dev; 
//     a02yyuw_reading_t r = {0};

//     a02yyuw_config_t cfg1 = a02yyuw_default_config(-1, A02_1_RX_GPIO, -1);
//     cfg1.use_sw_uart = true;
//     cfg1.baudrate = 9600;

//     a02yyuw_config_t cfg2 = a02yyuw_default_config(-1, A02_2_RX_GPIO, -1);
//     cfg2.use_sw_uart = true;
//     cfg2.baudrate = 9600;

//     while (1) {
//         // ========== 监听 1 号传感器 ==========
//         if (a02yyuw_init_dev(&dev, &cfg1) == ESP_OK) {
//             // 【关键修改】：超时时间拉长到 300ms，确保必定能覆盖一个完整的数据帧
//             if (a02yyuw_read_dev(&dev, &r, 300) == ESP_OK && r.valid) {
//                 printf("[A02YYUW#1] distance=%d mm\n", r.distance_mm);
//             } else {
//                 printf("[A02YYUW#1] read error or timeout (RX=GPIO%d)\n", A02_1_RX_GPIO);
//             }
//             a02yyuw_deinit_dev(&dev);
//         }
        
//         vTaskDelay(pdMS_TO_TICKS(50)); 

//         // ========== 监听 2 号传感器 ==========
//         if (a02yyuw_init_dev(&dev, &cfg2) == ESP_OK) {
//             if (a02yyuw_read_dev(&dev, &r, 300) == ESP_OK && r.valid) {
//                 printf("[A02YYUW#2] distance=%d mm\n", r.distance_mm);
//             } else {
//                 printf("[A02YYUW#2] read error or timeout (RX=GPIO%d)\n", A02_2_RX_GPIO);
//             }
//             a02yyuw_deinit_dev(&dev);
//         }
...
// void app_main(void)
// {  
//     // 【新增这行】：全局安装一次 GPIO 中断服务，抑制底层的重复安装报错
...
//     /* ---- launch all sensor tasks -------------------------------*/
//     // 【修改】：原本两个独立的超声波任务，现在合并为了一个共享任务
//     xTaskCreate(task_a02yyuw_shared, "a02_shared", 4096, NULL, 1, NULL);
...
//     printf("All 6 sensor tasks launched.\n");
// }