 ...
 * @brief       播放某个wav文件
 * @param       fname : 文件路径+文件名
 * @param       breakpoint : 当前歌曲的断点记忆位置指针 (【新增】参数)
 * @retval      KEY0_PRES : 下一首
 * KEY1_PRES : 上一首
 * KEY2_PRES : 停止/启动
 * 其他,非WAV文件
 */
uint8_t wav_play_song(uint8_t *fname, uint32_t *breakpoint)
{
    uint8_t key = 0;
    uint8_t res = 0;
    ...
    if (g_audiodev.file || g_audiodev.tbuf)
    {
        ...
        if (res == 0)                                               /* 解码成功 */
        {
            ...
            if (res == FR_OK)
            {
                /* 【新增】断点记忆：如果存在断点，则直接跳转到断点位置 */
                if (*breakpoint > 0)
                {
                    f_lseek(g_audiodev.file, *breakpoint);
                }
                /* 注意：如果 breakpoint == 0，wav_decode_init 已经正确解析了头部，无需跳转 */
                ...
                { 
                    while (1)
                    {
                        ...
                        {
                            *breakpoint = 0; /* 【新增】歌曲正常播放到底，清空断点记忆 */
                            res = KEY0_PRES;
                            break;
                        }
                    ...
                    {
                        /* 【新增】用户主动切歌，记录此时的文件读取指针作为断点 */
                        *breakpoint = f_tell(g_audiodev.file);
                        
                        res = key;
                        break;
                    }
                    else if (i2s_play_end == ESP_OK)                            /* 自然播放结束退出 */
                    {
                        res = KEY0_PRES;
                        break;
                    }
