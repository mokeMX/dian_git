...
/* ================================================================
 * Pin / hardware constants  (YD-ESP32-S3 V1.4)
 *
 * 适配 YD-ESP32-S3 核心板。该板无板载 I2C 外设，I2C 改用软件
 * (bit-bang) GPIO 模拟实现，共享同一对 SDA/SCL 引脚。
 * ================================================================ */
#define HUB_I2C_SDA_GPIO      11
#define HUB_I2C_SCL_GPIO      12
#define HUB_I2C_SPEED_HZ      100000
...
/* ---- shared resources ------------------------------------------*/
static sw_i2c_t g_shared_sw_i2c;
...
/* ---- VL53L1X ToF (Software I2C, addr 0x52) ---------------------*/
...
void app_main(void)
{
    printf("\nAutobox sensor hub test start (YD-ESP32-S3 V1.4, SW-I2C)\n");
    printf("SW-I2C: SDA=GPIO%d SCL=GPIO%d speed=%d Hz\n",
           HUB_I2C_SDA_GPIO, HUB_I2C_SCL_GPIO, HUB_I2C_SPEED_HZ);

    /* ---- shared SW-I2C bus -------------------------------------*/
    print_status("shared_sw_i2c",
                 sw_i2c_init(&g_shared_sw_i2c, HUB_I2C_SDA_GPIO, HUB_I2C_SCL_GPIO,
                             HUB_I2C_SPEED_HZ));
